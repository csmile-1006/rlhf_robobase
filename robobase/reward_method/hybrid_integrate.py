...
class HybridReward(RewardMethod):
            ...
            with torch.no_grad():
                if member == -1:
                    ...
                    for mem in range(self.num_reward_models):
                        ...
                        _scaled_reward_weights = self.weight_tuner.transform_to(
                            _reward_weights
                        )
                    ...
                    scaled_reward_weights = self.weight_tuner.transform_to(
                        _reward_weights
                    )
                    if self.reward_operator == "sum":
                        weighted_reward = (
    ...
    def update(self, batches: TensorDict) -> dict:
        ...
        for mem in range(self.num_reward_models):
            ...
            for i in range(2):
                ...
                normalized_weight = self.weight_tuner.transform_to(raw_weight)
    ...
    def early_stopping_criteria(self, metrics: dict) -> bool:
        pref_acc = metrics["pref_acc_label_0"]
        return pref_acc > 0.97
